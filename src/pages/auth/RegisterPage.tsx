...
function PasswordStrength({ password }: { password: string }) {
  ...
  const checks = [
    { label: t("auth.register.strength.minChars"), ok: password.length >= 8 },
    { label: t("auth.register.strength.uppercase"), ok: /[A-Z]/.test(password) },
    { label: t("auth.register.strength.number"), ok: /[0-9]/.test(password) },
    { label: t("auth.register.strength.symbol"), ok: /[^A-Za-z0-9]/.test(password) },
  ];
      ...
      <div className="flex gap-1">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className={cn(
            ...
            i < score
              ? score === 4 ? "bg-cost-positive shadow-[0_0_6px_hsl(142_71%_45%)]"
                : score >= 2 ? "bg-primary shadow-[0_0_6px_hsl(15_90%_55%)]"
                : "bg-cost-warning"
              : "bg-secondary"
          )} />
        ))}
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1">
        {checks.map((c) => (
          <div key={c.label} className="flex items-center gap-1.5 text-[11px]">
            {c.ok ? <Check className="w-3 h-3 text-cost-positive" /> : <X className="w-3 h-3 text-muted-foreground" />}
...
export default function RegisterPage() {
  ...
  const schema = z.object({
    fullName: z.string().trim().min(2, t("auth.register.errors.minName")).max(80),
    ...
    message: t("auth.register.errors.noMatch"),
    path: ["confirmPassword"],
  });

  useEffect(() => {
    if (!authLoading && user) navigate(pendingRedirect ?? "/", { replace: true });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!parsed.success) {
      ...
      setErrors({
        fullName: fld.fullName?.[0] || "", email: fld.email?.[0] || "",
        password: fld.password?.[0] || "", confirmPassword: fld.confirmPassword?.[0] || "",
      });
      return;
    }
    setLoading(true);
    ...
    if (error) {
      if (error.message.toLowerCase().includes("already")) {
        toast.error(t("auth.register.errors.alreadyExists"), { description: t("auth.register.errors.loginInstead") });
      } else {
        toast.error(t("auth.register.errors.couldNotCreate"), { description: error.message });
      }
      return;
    }
    toast.success(t("auth.register.success.created"), { description: t("auth.register.success.ready") });
    if (data.session) { setPendingRedirect("/"); return; }
  ...
  const handleGoogle = async () => {
    ...
    if (error) { setLoading(false); toast.error(t("auth.login.errors.googleError"), { description: error.message }); }
  };
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
        <div className="space-y-1.5">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">{t("auth.register.fullName")}</Label>
          <div className="relative">
            <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input type="text" placeholder={t("auth.register.fullNamePlaceholder")} value={fullName}
        ...
        <div className="space-y-1.5">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">{t("auth.login.passwordLabel")}</Label>
          <div className="relative">
        ...
        <div className="space-y-1.5">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">{t("auth.register.confirmPassword")}</Label>
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input type={showPwd ? "text" : "password"} placeholder="••••••••" value={confirmPassword}
        ...
        <Button type="submit" disabled={loading} className="w-full h-11 fire-button font-semibold mt-2">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
            <><UserPlus className="w-4 h-4" />{t("auth.register.submit")}</>
          )}
        </Button>
      </form>
    </AuthLayout>
